(function(){
    ...
    const inventors = [
      ...
      const earlyBornInventors = inventors.filter((inventor) => inventor.year < 1600 && inventor.year > 1499);
      console.table(earlyBornInventors);
      ...
      const sortInventorsByBirthdate = inventors.sort((a, b) => a.year > b.year);
      console.table(sortInventorsByBirthdate);
  
      // Array.prototype.reduce()
      // 4. How many years did all the inventors live all together?
      const inventorstotalYears = inventors.reduce((sum, inventor) => sum + (inventor.passed - inventor.year), 0);
      console.log(inventorstotalYears);
  
      // 5. Sort the inventors by years lived
      const sortInventorsByYearsLived = inventors.sort((a, b) => (a.passed - a.year) > (b.passed - b.year));
      console.table(sortInventorsByYearsLived);
  
      // 6. create a list of Boulevards in Paris that contain 'de' anywhere in the name
      // https://en.wikipedia.org/wiki/Category:Boulevards_in_Paris
      const listofBoulevardsInParis = [
        'Boulevards of Paris', 'City walls of Paris','Thiers wall','Wall of Charles V','Wall of Philip II Augustus',
      ...
      const filterBoulevardsInParis = listofBoulevardsInParis.filter((value) => value.includes('de'));
      console.log(filterBoulevardsInParis);
      ...
      const instancesSum = data.reduce((object, item) => (object[item] ? ++object[item] : object[item] = 1, object), {});
